...
int main(int argc, char *argv[]) {
	...
	matMulitCoreMultiplFloat(transposeX, colNum, rowNum, X, rowNum, colNum, tranXmulxMat);
	...
	matMulitCoreMultiplFloat(inv, colNum, colNum, transposeX, colNum, rowNum, resultM);
	...
	matMulitCoreMultiplFloat(resultM, colNum, rowNum, Y, rowNum, 1, finalResult);

	// cout<<"final (X'X)^-1*X'Y ->"<<endl;
	// display(finalResult, colNum, 1);

	time_taken = ((double)(end - start))/ CLOCKS_PER_SEC;
	cout<<" multi Core Taken->"<<time_taken<<endl;
	return 0;
}
